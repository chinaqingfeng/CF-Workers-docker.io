...
// 由 Cloudflare Worker 版本改写而来
// 文件名 [[default]].js 表示 catch-all 路由，会接管所有未匹配静态资源的请求
...
async function searchInterface() {
	...
	<head>
		...
		* { box-sizing: border-box; margin: 0; padding: 0; }
		...
		.github-corner svg {
			fill: var(--github-bg-color); color: var(--github-color);
		...
		.search-container {
			display: flex; align-items: stretch; width: 100%; max-width: 600px;
			margin: 0 auto; height: 55px; position: relative;
			animation: slideInFromBottom 0.5s ease-out 0.6s both;
			box-shadow: 0 10px 25px rgba(0, 0, 0, 0.15); border-radius: 12px; overflow: hidden;
		}
		...
		@media (max-width: 768px) {
			.container { padding: 20px 15px; min-height: 60vh; }
			.title { font-size: 2em; }
			.subtitle { font-size: 1em; margin-bottom: 20px; }
			.search-container { height: 50px; }
		}
		@media (max-width: 480px) {
			.container { padding: 15px 10px; min-height: 60vh; }
			...
			.subtitle { font-size: 0.95em; margin-bottom: 18px; }
		}
		</style>
	</head>
	<body>
		...
		<div class="container">
			...
			<p class="tips">基于 EdgeOne Pages 构建，利用全球边缘网络实现毫秒级响应。</p>
		</div>
		<script>
...
// ============== EdgeOne Pages 入口 ==============
// 改造点 1：从 `export default { fetch(...) }` 改为 `export async function onRequest(context)`
// context 中包含 { request, env, params, waitUntil, next }
export async function onRequest(context) {
	...
	if (env && env.UA) 屏蔽爬虫UA = 屏蔽爬虫UA.concat(await ADD(env.UA));
	...
	if (url.pathname.includes('/token')) {
		// 改造点 2：移除 cacheTtl（EdgeOne Edge Functions fetch 不支持该选项）
		let token_parameter = {
			headers: {
				'Host': 'auth.docker.io',
